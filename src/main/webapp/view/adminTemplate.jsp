...
<script>
		...
		xmlHttp.onreadystatechange = function() {
			if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
				...
				if (url == "findTopSoldProducts") {
					...
					plotChart('#topProfitableProducts',
							'Top 5 Profitable Products', JSON.parse(JSON
									.stringify(names)), JSON.parse(JSON
									.stringify(quantities)))
					...
					plotChart('#topNonProfitableProducts',
							'Top 5 Less Profitable Products', JSON.parse(JSON
									.stringify(names)), JSON.parse(JSON
									.stringify(quantities)))
				} else {
					for (var i = 0; i < result.length; i++) {
	...
	function plotChart(id, chartName, productsLabel, productsSeries) {
		...
		var donut = new ApexCharts(document.querySelector(id),
	...
	function loadMonthlyProfit(labels, values) {
		...
		var chartArea = new ApexCharts(
				document.querySelector('#monthlyProfit'), optionsArea);
		chartArea.render();
	}
</script>
</html>